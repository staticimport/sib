...
template <typename T>
static void verify_equal(sib::array_hash_set<T> const& sib_set,
...
{
  sib::array_hash_set<T> set;
  std::set<T> std_set;
  for(std::size_t ii = 1; ii < data.size(); ++ii) {
    std::pair<typename sib::array_hash_set<T>::iterator, bool> r1 = set.insert(data[ii]);
    std_set.insert(data[ii]);
    EXPECT_TRUE(r1.second);
    EXPECT_TRUE(data[ii] == *r1.first);
    verify_equal<T>(set, std_set);
    std::pair<typename sib::array_hash_set<T>::iterator, bool> r2 = set.insert(data[ii]);
    std_set.insert(data[ii]);
    EXPECT_FALSE(r2.second);
    EXPECT_TRUE(data[ii] == *r2.first);
    verify_equal<T>(set, std_set);
  }
}
...
{
  sib::array_hash_set<T> set1;
  sib::array_hash_set<T> set2;
  for(std::size_t ii = 0; ii != data.size(); ++ii)
    set1.insert(data[ii]);
  set1.swap(set2);
...
static void test_assignment(std::vector<T> const& data)
{
  ...
  for(std::size_t ii = 0; ii != data.size(); ++ii)
    set1.insert(data[ii]);
  set2 = const_cast<sib::array_hash_set<T> const&>(set1);
  EXPECT_EQ(set1.size(), set2.size());
  typename sib::array_hash_set<T>::const_iterator i1 = set1.cbegin();
  typename sib::array_hash_set<T>::const_iterator i2 = set2.cbegin();
  for(std::size_t ii = 0; ii != data.size(); ++ii)
    EXPECT_TRUE(*(i1++) == *(i2++));
}
...
static void test_copy(std::vector<T> const& data)
{
  ...
  for(std::size_t ii = 0; ii != data.size(); ++ii)
    set1.insert(data[ii]);
  sib::array_hash_set<T> set2(const_cast<sib::array_hash_set<T> const&>(set1));
  EXPECT_EQ(set1.size(), set2.size());
  typename sib::array_hash_set<T>::const_iterator i1 = set1.cbegin();
  typename sib::array_hash_set<T>::const_iterator i2 = set2.cbegin();
  for(std::size_t ii = 0; ii != data.size(); ++ii)
...
static void test_lookup(std::vector<T> const& data)
{
  ...
  for(std::size_t ii = 1; ii != data.size(); ++ii)
    set.insert(data[ii]);
  sib::array_hash_set<T> const& const_ref = set;
  for(std::size_t ii = 1; ii != data.size(); ++ii) {
